...
table_prov = idata_child %>%
  group_by(PR) %>%
  mutate(total = n()) %>% 
  group_by(PR,total,N_Two_or_more) %>%
  summarize(N=n()) %>% 
    mutate( Percent = percent(N/total,1))%>% 
    filter(N_Two_or_more != "1")  %>%
  select(-N_Two_or_more)

table_city = idata_child %>%
  group_by(PR,CMA,N_Two_or_more) %>%
  summarize(N_city=n()) %>%
  
    mutate( Percent_city = percent(N_city/sum(N_city),1)) %>%
  ungroup() %>%
mutate(      CMA = ifelse(CMA =="Other census metropolitan areas, census agglomerations and o","_other",as.character(CMA)))%>% 
    filter(N_Two_or_more != "1")  %>%
  select(-N_Two_or_more) %>%
  arrange(CMA,-N_city) %>% filter(N_city >250 ) 
table_poster = full_join( table_prov, table_city %>% rename(N= N_city, Percent = Percent_city) ) %>% select(PR, CMA,total, N, Percent) %>%
  arrange(PR)
...
province_map_data = full_join( 
                      province_data %>% 
                        subset(CMA =="Other census metropolitan areas, census agglomerations and o") , province_map, 
                      by=c("PR" ="id"))
...
```{r}
gma_labels = gma_map_data %>% group_by(CMA.old) %>% summarize(
  # y
  long = mean(long), 
  # x 
  lat = mean(lat)) %>%
  mutate(short_label = gsub(" – .*","",CMA.old),
         short_label = gsub(" / .*","", short_label))

add_labels = tibble(CMA.old = "Grand Sudbury", lat = 1250604.3, long = 7085883 )
...
gma_labels_adj = gma_labels %>%
  mutate(city = ifelse(CMA.old == "Ottawa – Gatineau", "Ottawa", CMA.old ),
    ...
    GTA = case_when(
      CMA.old %in% GTA_cities[1:2] ~ "GTA-south",
      CMA.old %in% GTA_cities ~ "GTA-west",
      TRUE ~NA_character_),
...
geom_map(data=gma_map_data, 
  ...
  # define limits for the plot (since province_map and canada_map_areas uses same scale, can use either one...)
   ...
   theme(legend.position = c(0.71, 0.55))
...
essential_gma_labels = gma_labels %>% filter(short_label %in% good_labels)
map_plot + 
  # Montreal, Ottawa,Toronto
geom_label_repel(data          = subset(essential_gma_labels,  long > cutoff),
 ...
 ggsave(here::here("03_output","01_figures","canada_map_most_labels.png"), width = 13, height =7)
