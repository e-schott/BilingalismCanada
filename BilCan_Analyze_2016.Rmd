...
```{r}
# 2011 data files
#if(!exists("raw_idata_2011")) {
#raw_idata_2011 = read.spss("01_datasets/NHS-99M001X-E-2011-pumf-individuals_F1.sav", to.data.frame = T, use.value.labels = F)
#}
# convert strings to factors 
#idata_2011 <- lapply(raw_idata_2011, 
#                    Int2Factor)
# convert list to dataframe
#idata_2011 <- as.data.frame(idata_2011, 
...
#idata_short_2011 = idata_2011 %>% select(variables.keep)
#dfSummary(idata_short_2011) %>% view()
```
...
```{r}
idata_long$gma.interest = ifelse(idata_long$CMA %in% c("Vancouver", "Montréal", "Toronto","Ottawa - Gatineau" , "Calgary"),as.character(idata_long$CMA), NA)
```
...
# prep for circle plots
```{r}
# specific language combos
idata_child_lang_combo = idata_child %>% 
  filter(N_Two_or_more > 1 ) %>%
  group_by(PR,Lang_Combo_top2) %>%
  summarize(N=n())%>%
  mutate( Percent = round(N/sum(N),3))%>%
  arrange(PR, -N)

idata_child_lang_combo = idata_child_lang_combo %>%
  filter(PR %in% c("British Columbia","Alberta","Quebec", "Ontario")) %>%
  group_by(PR) %>%
  slice(1:5) %>%
...
idata_child_lang_combo = left_join(idata_child_lang_combo, dict)
...
gma_LangGroup_data_other = gma_LangGroup_data %>%    filter(substr(CMA,1,5) == "Other")

gma_LangGroup_data_other$PR= as.character(gma_LangGroup_data_other$PR)
plot.data = left_join(data, gma_LangGroup_data_other %>% subset(CMA =="Other census metropolitan areas, census agglomerations and o") , by=c("id" ="PR"))
...
hello = stringdist_left_join(gma_LangGroup_data, map_areas2016 %>% rename(CMA = id), by ="CMA", distance_col = NULL)
hello %>% select(PR,CMA.x, CMA.y) %>% filter(is.na(CMA.y)) %>% write.csv(., file=here("02_map_data/Dict-CMA.csv"), row.names=F) 
dict.map  = read.csv(here("02_map_data/Dict-CMA-coded.csv"))
gma_LangGroup_data = gma_LangGroup_data %>% left_join(dict.map) %>% rename(CMA.old = CMA) %>%  mutate(CMA = ifelse(is.na(CMA.splitUp), CMA.old, as.character(CMA.splitUp))) %>% select(-CMA.old, -CMA.splitUp)

  test = gma_LangGroup_data %>% stringdist_left_join(map_areas2016 %>% rename(CMA = id), by = "CMA", distance_col = NULL) %>% select(-CMA.old, -CMA.x, -area) %>%
    rename(id = CMA.y)
