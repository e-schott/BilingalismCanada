...
# get number of languages per household
adults_lang_n <- hhdata_child_adult %>% 
  select(household_id, lang1:lang4) %>% 
  pivot_longer(lang1:lang4, names_to = "lang_number", values_to = "lang_name") %>% 
  filter(!is.na(lang_name)) %>% 
  group_by(household_id) %>% 
  summarize(lang_n_hh = n_distinct(lang_name))
...
adults_monoling_n <- adults_lang %>% 
  filter(!str_detect(lang_pair, "&")) %>% 
  mutate(lang_pair = na_if(lang_pair, "NA")) %>% 
  filter(!is.na(lang_pair)) %>% 
  group_by(household_id) %>% 
  summarize(n_monoling_adults = sum(n_lang_pair))


# get languages per household
adults_lang_hh <- hhdata_child_adult %>% 
  select(household_id, lang_pair) %>% 
  separate(lang_pair, into = c("lang1", "lang2", "lang3", "lang4"), sep = "&", remove = FALSE) %>% 
  pivot_longer(lang1:lang4, names_to = "lang_n", values_to = "lang_spoken", values_drop_na = TRUE) %>% 
  group_by(household_id, lang_spoken) %>%
  summarize(lang_hold = n()) %>% 
  select(-lang_hold) 

# identify households with both French and English spoken
eng_hh <- adults_lang_hh %>% 
  filter(lang_spoken == "English")

fr_hh <- adults_lang_hh %>% 
  filter(lang_spoken == "French")

eng_fr_hh <- eng_hh %>% 
  filter(household_id %in% fr_hh$household_id) %>% 
  select(household_id)
...
adults_n <- adults_n %>% 
  ...
  left_join(adults_lang_n, by = "household_id") %>% 
  ...
  #add true/false column for Fr-Eng biling exposure
  mutate(fr_eng = case_when(
    household_id %in% eng_fr_hh$household_id ~ TRUE,
    TRUE ~ FALSE
    ))
...
## create df for children only (for predictor analysis)
