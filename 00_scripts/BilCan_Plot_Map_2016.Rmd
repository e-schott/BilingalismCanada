...
provinces_with_no_cma = c("Newfoundland and Labrador", "Prince Edward Island", "Northern Canada")

geo_data  = geo_data %>% full_join(northern_data) %>%
  mutate(type_data = ifelse(area == "", "province", "area")) %>%
  select(-id, -country) %>% 
