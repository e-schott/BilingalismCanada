...
table_prov = data_table2 %>%
  ...
  mutate(total = n()) %>% 
  group_by(province, age_collapsed, total, n_languages) %>%
  summarize(N=n()) %>% 
...
combined_table = full_join( table_city, table_prov) %>%
  full_join(total_across_canada) %>%
  select(province, area, age_collapsed, Percent) %>%
  pivot_wider(values_from = Percent, names_from = age_collapsed) %>%
  arrange(province, area) %>% View()
...
```{r}
table_prov %>%  
  select(-total, -N_bilingual) %>% 
  pivot_wider(values_from = Percent, names_from = age_collapsed) %>% 
  mutate(rank_0_4 = rank(age_0_to_4)) %>% 
  arrange(age_5_to_9)
```
