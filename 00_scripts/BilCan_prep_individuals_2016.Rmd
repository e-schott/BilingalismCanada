...
lang_pair_data %>% 
  filter(age_group == "0 to 4 years") %>% 
  group_by(province) %>% 
  mutate(language_pair_simplified = fct_lump_min(as.factor(language_pair), min = 10, other_level = "z_other")) %>% # create other category
  count(language_pair_simplified) %>% 
  mutate(percent = round(n/sum(n)*100)) %>% # add percent
...
# 0-9 year olds
lang_pair_data %>% filter(age_group %in% c("0 to 4 years", "5 to 6 years", "7 to 9 years")) %>% group_by(province)%>% count(language_pair) %>% mutate(percent = round(n/sum(n)*100)) %>% arrange(province,-percent) %>% top_n(10) %>% View()
