...
xxx actually maybe it would make sense to have a separate calculation about how many languages people speak "most" and how many they use "regularly"
```{r}
...
# collapse the language pairings with less than 10 ppl into an "other" category
# exclude provinces where all language pairings have less than 10 ppl
# write as csv
lang_pair_data %>% 
  filter(age_group == "0 to 4 years") %>% 
  group_by(province) %>% 
  mutate(language_pair_simplified_full = fct_lump_min(as.factor(language_pair), min = 10, other_level = "Other Language Pairings"),
         language_pair_simplified_shortened = fct_lump_n(as.factor(language_pair_simplified_full), n = 10, other_level = "Other Language Pairings")) %>% # create other category
  count(language_pair_simplified_shortened) %>%  
  mutate(percent = round(n/sum(n)*100),
         other_flag = ifelse(language_pair_simplified_shortened =="Other Language Pairings",1,0)) %>% # add percent
  arrange(province, other_flag, -percent) %>% 
  filter(percent != 100) %>% # remove provinces that have only the other category
  select(-other_flag) %>%
