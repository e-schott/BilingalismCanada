...
lang_pair_by_province = 
  ...
  count(language_pair) %>%  
  mutate(percent = round(n/sum(n)*100),
         other_flag = str_detect(language_pair, "Other")) %>% # add percent
  arrange(province, -percent) %>% 
  filter( other_flag != TRUE)%>%
  select(-other_flag, - n) %>%
  spread(age_collapsed, percent) %>%
  rowwise() %>%
  mutate(average_percent = mean(c(age_0_to_4,  age_5_to_9), na.rm = TRUE),
         language_pair = gsub("&", " & ", language_pair)) %>% 
  filter(average_percent >= 4, !is.na(age_5_to_9)) %>%
  select(-average_percent) %>%
  arrange(province, -age_0_to_4)
