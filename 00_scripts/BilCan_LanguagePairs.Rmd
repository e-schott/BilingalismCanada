...
totals_across_canada = lang_pair_by_province_and_age_long %>% 
  ...
  group_by(province, age_collapsed) %>%
  top_n(n, n = 1) %>%
  select(country, province, age_collapsed,  total_bilingual_children, total_by_age)%>%
  group_by(country, age_collapsed) %>%
  summarize(total_by_age_canada = sum(total_by_age),
            n_bilingual_by_age_canada = sum(total_bilingual_children))
```
### calculate percent
```{r}
...
canada_language_pairs_dictionary =  canada_language_pairs %>% 
  ...
  ungroup() %>% 
  mutate(collapse = ifelse(n < 526, T, F),
    official_language = str_extract(language_pair_simplified, "English|French"),
         language_pair_collapsed = case_when(
          collapse == FALSE & !str_detect(language_pair_simplified, "Other") ~ language_pair_simplified,
           ...
           str_detect(language_pair_simplified, "Indigenous") ~ language_pair_simplified,
           official_language == "English" ~ "English & Other",
           official_language == "French" ~ "French & Other",
           is.na(official_language) ~ "Other Pairs"
         ),
         language_pair_type = case_when(
           str_detect(language_pair_simplified, "Indigenous") ~ "actual",
