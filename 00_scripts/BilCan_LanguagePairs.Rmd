...
This combines all language pairs with fewer than .5% of all children as speakers, and with less than 10 children (to avoid calculting % for unreliable small-sample data)
...
province_language_pair_data = province_language_pair_data %>%
  ...
  select(-n, -total_by_age) %>% # get rid of unnecessary column names
...
```
### create "All other pairs" catchall category

```{r}
table_2.2 = province_language_pair_data %>% 
  mutate(lang_pair_collapsed = case_when(
        province %in% c("Ontario", "Quebec", "Alberta") & str_detect(language_pair_simplified, "Indi") ~ "All other pairs",
order_by_type == 1 ~ language_pair_simplified,
    TRUE ~ "All other pairs"
  )) %>%
  group_by(province, lang_pair_collapsed) %>% 
  summarize(
    percent_bilingual_children_age_0_to_4 = sum(percent_bilingual_children_age_0_to_4, na.rm = TRUE),
    percent_bilingual_children_age_5_to_9 = sum(percent_bilingual_children_age_5_to_9, na.rm = TRUE),
    percent_bilingual_children_age_0_to_9 = sum(percent_bilingual_children_age_0_to_9, na.rm = TRUE),
    percent_all_children_age_0_to_4 = sum(percent_all_children_age_0_to_4, na.rm = TRUE),
    percent_all_children_age_5_to_9 = sum(percent_all_children_age_5_to_9, na.rm = TRUE),
    percent_all_children_age_0_to_9 = sum(percent_all_children_age_0_to_9, na.rm = TRUE)) %>% 
  mutate(order_by_type = ifelse(str_detect(lang_pair_collapsed, "other" ),2,1)) %>%
  arrange(province, order_by_type, -percent_bilingual_children_age_0_to_9)  %>%
  select(-order_by_type)
...
combined_table %>%
  write_excel_csv(here("03_output", "02_tables", "Tables_language_pairs_for_dashboard.csv"))
