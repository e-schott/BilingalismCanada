...
language_pair_dictionary = 
  ...
  add_row(language_pair = "Other Pairs", language_pair_old = "Other Pairs")
...
### create simplified language pairs table
some language pairs have very few speakers, like "English&German" in Newfoundland, therefore we combine those into a general "Other Pairs" category to shorten table 
This combines all language pairs with fewer than 0.5% of all children as speakers

```{r simplified_province}

province_lang_pair_dict = lang_pair_by_province_all_ages_combined %>%
  mutate(language_pair_simplified = case_when(
    str_detect(language_pair, "Other") ~ "Other Pairs",
    percent_all_children_age_0_to_9 > 0.5 ~ language_pair,
    TRUE ~ "Other Pairs")) %>% 
  select(-percent_bilingual_children_age_0_to_9, -percent_all_children_age_0_to_9)
```

### re-calculate percent with new "other pairs" category
```{r recalculate_other}
...
lang_pair_by_province_and_age_long  = 
  lang_pair_data %>% 
  select(province, age_group, language_pair, age_collapsed) %>%
...
province_language_pair_data = province_language_pair_data %>%
  ...
  arrange(province, order_by_type, -percent_bilingual_children_age_0_to_9) %>%
   left_join(language_pair_dictionary, by = c("language_pair_simplified" = "language_pair_old")) %>% 
  select(-language_pair_simplified, -n, -total_by_age, -order_by_type) %>% # get rid of messy column name
...
canada_language_pairs = lang_pair_by_province_and_age_long %>% 
  ...
  group_by(country, age_collapsed, language_pair_simplified, total_by_age_canada, n_bilingual_by_age_canada) %>%
...
```
### simplify language pairs
some language pairs have very few speakers, like "English&German". therefore we combine those into a general "Other Pairs" category to shorten table 
This combines all language pairs with fewer than 600 children as speakers 
...
canada_language_pairs_dictionary =  canada_language_pairs %>% 
  ...
  group_by(language_pair_simplified, age_combined) %>%
  summarize(n = sum(n_canada)) %>%
  ungroup() %>%
  mutate(collapse = ifelse(n > 600, F, T),
    official_language = str_extract(language_pair_simplified, "English|French"),
         language_pair_collapsed = case_when(
          collapse == FALSE & !str_detect(language_pair_simplified, "Other") ~ language_pair_simplified,
      
           str_detect(language_pair_simplified, "Aboriginal") ~ language_pair_simplified,
         ...
         language_pair_type = case_when(
           str_detect(language_pair_simplified, "Aboriginal") ~ "actual",
           str_detect(language_pair_simplified, "Other") ~ "collapsed", 
...
canada_ages_combined = canada_language_pairs %>%
  ...
  select(-language_pair_simplified) %>%
...
combined_table = lang_pair_by_area_wide %>% mutate(type = "cma", region = area) %>%
  full_join(province_language_pair_data %>% mutate(type = "province", region = province) %>% rename(language_pair_collapsed = language_pair)) %>%
