...
idata_child_lang_combo = idata_child_lang_combo %>%
  filter(PR %in% c("British Columbia","Alberta","Quebec", "Ontario", "Northern Canada"), N >5) %>%
  group_by(PR) %>%
  slice(1:5) %>%
...
ggsave(here("03_output","01_figures",paste("plot_",circle_plots$PR[[plot_num]],".png",sep="")), width=3.5,height=circle.plot.height)
}
```
